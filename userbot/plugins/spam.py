        ...
        if BOTLOG:
            if e.is_private:
                await e.client.send_message(
                    BOTLOG_CHATID,
                    "#SPAM\n"
                    + f"Spam foi executado com sucesso em [User](tg://user?id={e.chat_id}) chat with {counter} mensagens de \n"
                    + f"`{spam_message}`",
                )
            else:
                await e.client.send_message(
                    BOTLOG_CHATID,
                    "#SPAM\n"
                    + f"Spam foi executado com sucesso em {e.chat.title}(`{e.chat_id}`) chat  com {counter} mensagens de \n"
                    + f"`{spam_message}`",
                )
...
CMD_HELP.update(
    {
        "spam": ".cspam <text>\
\nUso: Spam o texto letra por letra.\
\n\n.spam <count> <text>\
...
\nUso: .delayspam mas com atraso personalizado.\
\n\n\n**NOTE : Use por sua própria conta e risco !!**"
    }
)

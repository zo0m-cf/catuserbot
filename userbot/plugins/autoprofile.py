...
async def _(event):
    await event.edit(f"Auto bio foi iniciado")
    while True:
...
\nSet `DOWNLOAD_PFP_URL_CLOCK` in the heroku vars first with telegraph link of required image\
\n\n**Syntax : **`.autoname`\
\n**Uso : **for time along with name, you must set `AUTONAME`in the heroku vars first for this to work \
\n\n**Syntax : **`.autobio`\
\n**Uso : **for time along with your bio, Set `DEFAULT_BIO` in the heroku vars first \
\n\n**Syntax : **`.monkeybio`\
\n**Uso : **sets funny monkey bio \

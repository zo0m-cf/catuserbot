    ...
    if G_DRIVE_DATA is not None:
        try:
            ...
            await edit_or_reply(
                ...
                "**AUTHENTICATE - ERROR**\n\n"
                "**Status : **`BAD`\n"
                "**Razão : **`G_DRIVE_DATA entity is not valid!`",
            )
            return False
    else:
        """ - Only for old user - """
        if G_DRIVE_CLIENT_ID is None and G_DRIVE_CLIENT_SECRET is None:
            await edit_or_reply(
                ...
                "**AUTHENTICATE - ERROR**\n\n"
                "**Status : **`BAD`\n"
                "**Razão : **`please get your G_DRIVE_DATA`",
            )
            return False
        configs = {
            "installed": {
        ...
        if isfile(required_file_name):
            ...
            try:
                ...
                reply += (
                    ...
                    "**Status : **`OK - Carregado com sucesso.`\n\n"
                )
                await reset_parentId()
                return reply
    ...
    if checker is not None:
        ...
        if page_size > 1000:
            await edit_or_reply(
                ...
                "**GDRIVE - LIST**\n\n"
                "**Status : **`BAD`\n"
                "**Razão : **`can't get list if limit more than 1000.`",
            )
            return
    else:
...
async def google_drive_managers(gdrive):
    ...
    gdrive = await edit_or_reply(gdrive, "`Enviando informações...`")
    reply = ""
...
async def google_drive(gdrive):
    ...
    if not value and not gdrive.reply_to_msg_id:
        ...
        await edit_or_reply(
            ...
            "**[UNKNOWN - ERROR]\n\n"
            "**Status : **`failed`\n"
            "**Razão : **`Confused to upload file or the replied message/media.`",
        )
        return None
    service = await create_app(gdrive)
    event = gdrive
    ...
    if isfile(value):
        ...
        if not uri and not event.reply_to_msg_id:
            await gdrive.edit(
                "**[VALUE - ERROR]**\n\n"
                "**Status : **`BAD`\n"
                "**Razão : **given value is not URL nor file/folder path. "
...
async def set_upload_folder(gdrive):
    ...
    if exe == "gdfclear":
        if G_DRIVE_FOLDER_ID is not None:
            ...
            await gdrive.edit(
                "**[FOLDER - SET]**\n\n"
                "**Status : **`OK- usando G_DRIVE_FOLDER_ID agora.`"
            )
            return None
        else:
            try:
    ...
    try:
        ...
        if True in [c1 or c2]:
            parent_Id = inp
            await gdrive.edit(
                "**[PARENT - FOLDER]**\n\n" "**Status : **`OK - Alterado com sucesso.`"
            )
            return None
        else:
            await gdrive.edit(
        ...
        if "uc?id=" in ext_id:
            await gdrive.edit(
                "**[URL - ERROR]**\n\n" "**Status : **`BAD - URL de pasta inválido.`"
            )
            return None
        try:
...
CMD_HELP.update(
    {
        ...
        "\n\n**Syntax : **`.gauth`"
        "\n**Uso : **generate token to enable all cmd google drive service."
        "\nThis only need to run once in life time."
        "\n\n**Syntax : **`.greset`"
        "\n**Uso : **reset your token if something bad happened or change drive acc."
        "\n\n**Syntax : **`.ugd`"
        "\n**Uso : **Upload file from local or uri/url/drivelink into google drive."
        "\nfor drivelink it's upload only if you want to."
        "\n\n**Syntax : **`.gabort`"
        "\n**Uso : **Abort process uploading or downloading."
        "\n\n**Syntax : **`.gdf mkdir`"
        "\n**Uso : **Create gdrive folder."
        "\n\n**Syntax : **`.gdf chck`"
        "\n**Uso : **Check file/folder in gdrive."
        "\n\n**Syntax : **`.gdf rm`"
        "\n**Uso : **Delete files/folders in gdrive."
        "\nCan't be undone, this method skipping file trash, so be caution..."
        "\n\n**Syntax : **`.gdfset`"
        "\n**Uso : **Change upload directory in gdrive."
        "\ninto **G_DRIVE_FOLDER_ID** and if empty upload will go to root."
        "\n\n**Syntax : **`.gdfclear`"
        "\n**Uso : **remove set parentId from cmd\n>`.gfset put` "
        "\n\n**Syntax : **`.glist`"
        "\n**Uso : **Get list of folders and files with default size 50."
